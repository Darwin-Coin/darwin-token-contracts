...
/**
 * Chainlink VRF v2 Consumer retrieving random numbers for SmartGamble's Roulette game.
 */
contract VRFv2Consumer is VRFConsumerBaseV2, ConfirmedOwner {
    ...
    function getRequestStatus(uint256 _requestId) external view returns (bool fulfilled, uint256 randomNum) {
        require(s_requests[_requestId].exists, "request not found");
        RequestStatus memory request = s_requests[_requestId];
        return (request.fulfilled, request.randomWords[0]);
    }

    function getRequest(uint256 _requestId) external view returns (bool exists, bool fulfilled) {
        return (s_requests[_requestId].exists, s_requests[_requestId].fulfilled);
    }
}